	...
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if info.IsDir() {
			...
			if _, err := os.Stat(indexPath); err == nil {
				serveFile(indexPath, w)
				if verbose {
					log.Printf("Serving file: %s", indexPath)
				}
				return
			}
			...
			fmt.Fprintf(w, "<html><head><meta charset=\"utf-8\"><title>Index of %s</title></head><body><h1>Index of %s</h1><ul>", requestedPath, requestedPath)
			for _, e := range entries {
				...
				fmt.Fprintf(w, `<li><a href="%s">%s</a></li>`, link, link)
			}
		...
		serveFile(fullPath, w)
	}
}

func serveFile(fullPath string, w http.ResponseWriter) {
	/*
